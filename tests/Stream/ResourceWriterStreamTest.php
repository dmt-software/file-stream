...
#[CoversClass(ResourceWriterStream::class)]
final class ResourceWriterStreamTest extends TestCase
{
    public function testExposeInternalBufferStream(): void
    {
        $resource = fopen('php://memory', 'w+');

        $stream = new ResourceWriterStream($resource);
    ...
    {
        $stream = new ResourceWriterStream(fopen('php://memory', 'w+'));
    ...
    public function testWriteBuffersData(): void
    {
        ...
        $stream = new ResourceWriterStream($resource, bufferSize: 1024);
        $stream->write('test');
    ...
    public function testFlushWritesBufferedData(): void
    {
        $resource = fopen('php://memory', 'w+');
        ...
        $this->assertSame('test', stream_get_contents($resource));
    }
    ...
    public function testBufferFlushesWhenLimitIsReached(): void
    {
        ...
        $stream = new ResourceWriterStream($resource, bufferSize: 4);
        $stream->write('test');
    ...
    public function testRejectInvalidBufferSize(): void
    {
        ...
        new ResourceWriterStream(fopen('php://memory', 'w+'), bufferSize: 0);
    }
}
